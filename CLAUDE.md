# Claude Code Project Instructions — similar-kotlin

## Project Overview

This is **similar-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`similar`](https://crates.io/crates/similar).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/similar/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/similar>
