...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Similar.swiftmodule/`
...
//      `import Similar` above would fail at compile time.
//
//   2. The static archive `libSimilar.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Similar" }` and
//      `flattenPackage = "io.github.kotlinmania.similar"` configuration in
...
//      via the `SimilarLibrary` product.
//
...
final class SimilarExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Similar swift module imported cleanly")
    }
}
